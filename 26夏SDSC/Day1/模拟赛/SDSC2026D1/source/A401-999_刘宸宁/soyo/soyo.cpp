...
ll n,x,y,f[N][2][2];vector<pii> g[N];
inline void dfs(ll x,ll fa,ll fw)
{
    if(x!=1&&g[x].size()==1) return f[x][0][0]=1,void();ll sum=1;
    vector<pii> vec;for(auto [y,w]:g[x]) if(y!=fa) vec.pb(mk(w,y)),dfs(y,x,w),_M(sum,A_(f[y][1][0],f[y][0][0]));
    sort(all(vec),greater<pii>());ll lasw=inf;for(auto [w,y]:vec)
        _A(f[x][lasw<fw&&fw!=-1][w>fw&&fw!=-1],sum),_M(sum,qpow(A_(f[y][1][0],f[y][0][0]),mod-2),A_(f[y][0][0],f[y][0][1])),lasw=w;
    _A(f[x][lasw<fw&&fw!=-1][0],sum);
}
bool ED;
signed main(){
    freopen("soyo.in","r",stdin);
    freopen("soyo.out","w",stdout);
    read(n);fo(1,i,n-1) read(x,y),g[x].pb(mk(y,i)),g[y].pb(mk(x,i));dfs(1,0,-1),wr(f[1][0][0]),pr;
