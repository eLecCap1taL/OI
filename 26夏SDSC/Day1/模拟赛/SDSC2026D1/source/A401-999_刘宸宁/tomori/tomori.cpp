...
const ll N=2e3+5,M=1e5+5,inv2=499122177;
ll n,a[N],b[N],f[N][N],_a[N],fa[N];
...
signed main(){
    ...
    read(n);fo(1,i,n) read(a[i]);InitC(n);
    if(n==1) return wr(a[1]),pr,0;
    if(n==2) return wr(M_(inv2,A_(a[1],a[2]))),pr,0;
    if(n==3)
    {
        ll sum1=M_(A_(M_(A_(a[1],a[2]),inv2),a[3]),inv2),sum2=M_(A_(M_(A_(a[2],a[3]),inv2),a[1]),inv2);
        wr(M_(A_(sum1,sum2),inv2)),pr;return 0;
    }
    if(n<=10)
    {
        fo(1,i,n-1) b[i]=i;fo(1,i,n) _a[i]=a[i];ll sum=0;
        do
        {
            fo(1,i,n) a[i]=_a[i],fa[i]=i;
            fo(1,i,n-1)
            {
                ll k1=a[Find(b[i])],k2=a[Find(b[i]+1)];
                Link(b[i],b[i]+1),a[Find(b[i]+1)]=M_(A_(k1,k2),inv2);
            }_A(sum,a[Find(1)]);
        }while(next_permutation(b+1,b+n));wr(M_(sum,qpow(Pre[n-1],mod-2))),pr;return 0;
    }
    fo(1,i,n) f[i][i]=a[i];
    fo(2,len,n) for(ll l=1,r=l+len-1;r<=n;++l,++r) fo(l,k,r-1)
        _A(f[l][r],M_(A_(M_(f[l][k],Pre[r-k-1]),M_(f[k+1][r],Pre[k-l])),inv2,C(r-l-1,k-l)));
    wr(M_(f[1][n],qpow(Pre[n-1],mod-2)));
