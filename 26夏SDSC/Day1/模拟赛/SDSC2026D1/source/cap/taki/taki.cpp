...
ostream& operator<<(ostream& os,Stone res){
	os<<"{"<<res.sum<<','<<res.val<<','<<res.N<<"}";
	return os;
}
...
struct infoUpdate{
	typedef Stone metadata_type;
	
	virtual cptr node_begin() const=0;
	virtual cptr node_end() const=0;
	
	LL get_ans(){
		// auto it=node_begin().m_p_nd;
		// while(it->m_p_parent->m_p_parent!=it)	it=it->m_p_parent;
		// return it->get_metadata().val;
		return node_begin().get_metadata().val;
	}
	
	void operator()(auto it,auto end_it){
		auto lc=it.get_l_child();
		auto rc=it.get_r_child();
		Stone res=(*it)->fi;
		if(lc!=end_it)	res.add_left(lc.get_metadata());
		if(rc!=end_it)	res.add_right(rc.get_metadata());
		const_cast<Stone&>(it.get_metadata())=res;
	}	
};
...
protected:
	...
	tree<pair<Stone,int>,null_type,less<pair<Stone,int>>,rb_tree_tag,infoUpdate> tr;
	int cnt;
	
public:
	void load(int tm,const Stone& res){
		tr.insert(mkp(res,++cnt));
		ls+=res;
	}
	void unload(int tm,const Stone& res){
		tr.erase(tr.lower_bound({res,0}));
	}
