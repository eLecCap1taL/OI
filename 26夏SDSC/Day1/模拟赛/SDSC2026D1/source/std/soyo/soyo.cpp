...
int f[MAXN][2]; //1 = to up; 0 = to down
int fa[MAXN];

int suf[MAXN];

void dfs(int u,int fath,int fav){
	fa[u]=fath;
	...
	suf[sz(e[u])+1]=1;
	for(int i=sz(e[u]);i>=1;i--){
		auto [v,w]=e[u][i-1];
		
		if(v==fa[u]){
			suf[i]=suf[i+1];
		}else{
			suf[i]=mul(suf[i+1],f[v][0]);
		}
	}
	...
	f[u][flag]=suf[1];
	for(int i=1;i<=sz(e[u]);i++){
		auto [v,w]=e[u][i-1];
		
		if(v==fa[u]){
			flag=0;
		}else{
			premul=mul(premul,f[v][1]);
		}
		
		f[u][flag]=add(f[u][flag],mul(premul,suf[i+1]));
	}
}
signed main()
{
	...
	//sort edge
	foru(u,1,n){
		sort(All(e[u]),[&](auto& x,auto& y)->bool{
			return x.se>y.se;
		});
	}
