...
const ll N=3e5+5,M=1e5+5;
ll k,q,s[N];deque<ll> a[N],b[N];
inline ll check()
{
    fo(1,i,k) b[i]=a[i];priority_queue<pii,vector<pii>,greater<pii>> pq;
    fo(1,i,k) if(b[i].size()) pq.push(mk(b[i].front(),i)),b[i].pop_front();
    ll sum=0,ans=0;while(pq.size())
    {
        ll k=pq.top().fi,id=pq.top().se;pq.pop();sum+=k,ans+=sum;
        if(b[id].size()) pq.push(mk(b[id].front(),id)),b[id].pop_front();
    }return ans;
}
bool ED;
signed main(){
    ...
    {
        read(s[i]);ll x;
        fo(1,j,s[i]) read(x),a[i].pb(x);
    }wr(check()),pr;
    fo(1,i,q)
    {
        ll op,t,v;read(op,t,v);
        if(op==0) a[t].push_front(v);
        if(op==1) a[t].push_back(v);
        wr(check()),pr;
    }
