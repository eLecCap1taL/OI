...
ll n,k,m,a[N],vis[N];vector<pii> vec;
inline void solve()
{
    read(n,k,m),k>>=1;fo(1,i,m) read(a[i]);ll las=1,sum=0;
    ll cnt=0;fo(1,i,m)
    {
        if(a[i]!=las)
        {
            ll opt=a[i]-las;
            while(opt>(k*2)) opt-=k*2;
            vec.pb({opt,++cnt}),sum+=opt;
        }
        las=a[i]+1;
    }
    if(las<=n)
    {
        ll opt=n-las+1;
        while(opt>(k*2)) opt-=k*2;
        vec.pb(mk(opt,++cnt)),sum+=opt;
    }
    if(sum%(2*k)!=0) return printf("NO\n"),void();if(!sum) return printf("YES\n"),void();
    sum>>=1;deque<pii> q1,q2;ll nw=0;
    for(auto [l,id]:vec)
    {
        // deb(l,id,k);
        if(nw<sum)
        {
            if(nw+l>sum)
                q1.pb(mk(sum-nw,id)),q2.pb(mk(l-(sum-nw),id));
            q1.pb(mk(l,id)),nw+=l;
        }
        else q2.push_back(mk(l,id)),nw+=l;
    }
    while(q1.size()&&q2.size())
    {
        vector<ll> ID;ll nw=k;
        while(nw) ID.pb(q1.front().se),(q1.front().fi>nw?q1.front().fi-=nw,nw=0,1:(nw-=q1.front().fi,q1.pop_front(),1));
        for(ll i:ID) vis[i]=1;nw=k;
        while(nw)
        {
            if(vis[q2.front().se]) return printf("NO\n"),void();
            (q2.front().fi>nw?q2.front().fi-=nw,nw=0,1:(nw-=q2.front().fi,q2.pop_front(),1));
        }
    }if(q1.size()||q2.size()) return printf("NO\n"),void();printf("YES\n");
}
inline void cls(){fo(0,i,n+1) a[i]=vis[i]=0;vec.clear();}
bool ED;
signed main(){
