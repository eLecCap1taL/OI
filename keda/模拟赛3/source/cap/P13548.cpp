...
class Kruskal{
	...
	KruskalTree* g;
public:
	void set(int _n,KruskalTree* _g){
		g=_g;
		n=_n;
...
public:
	...
	int find_left(int u,int x){
		...
		auto it=st[rt].upper_bound({x,INT_MAX});
		
		if(it==st[rt].begin())	return x-1;
		it--;
		
		if(it->se<x-1)	return x-1;
		return it->fi-1;
	}
	int find_right(int u,int x){
		...
		if(it==st[rt].begin()){
			if(it->fi==x+1)	return it->se+1;
			return x+1;
		}
		auto itl=prev(it);
		
		if(itl->se>=x)	return itl->se+1;
		
		if(it==st[rt].end())	return x+1;
		if(it->fi==x+1)	return it->se+1;
		return x+1;
	}
	int find_root(int u){
...
class Boruvka{
	...
	int n;
public:
	void set(int _n,KruskalTree* _g,Kruskal* _tar){
		n=_n;
		g=_g;
		tar=_tar;
		dsu._set(n);
	...
	void process(){
		...
		while(dsu.count()>1){
			...
			foru(i,1,n){
				mn[i]=Edge{-1,-1,INT_MAX};
			}
			
			// get mn
			foru(_u,1,n){
				int rt=dsu.find_root(_u);
				...
				_x=_u;
				while(1){
					_x=dsu.find_left(_u,_x);
					if(eg.exist(g->get_p(_u),g->get_p(_x)))	continue;
					if(_x<1)	break;
					
					//x!
					int val=g->query(g->get_p(_u),g->get_p(_x));
					if(val<mn[rt].w){
						mn[rt]=Edge{_u,_x,val};
					}
					break;
				}
			...
			// cout<<"new Union"<<endl;
			foru(_u,1,n){
				if(dsu.find_root(_u)!=_u)	continue;
				dsu.Union(mn[_u].u,mn[_u].v);
				
				
				// cout<<"root "<<u<<' '<<mn[u].u<<' '<<mn[u].v<<' '<<mn[u].w<<endl;
				
				// work with boruvka
				tar->add_edge(g->get_p(mn[_u].u),g->get_p(mn[_u].v),mn[_u].w);
			}
		}
