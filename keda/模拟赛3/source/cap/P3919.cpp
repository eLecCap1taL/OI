...
public:
	...
	void modify(int p1,int& p2,int l,int r,int pos,int k){
		tr[p2=++pcnt]=tr[p1];
		...
		int mid=(l+r)>>1;
		if(pos<=mid)	modify(tr[p1].lc,tr[p2].lc,l,mid,pos,k);
		else		modify(tr[p1].rc,tr[p2].rc,mid+1,r,pos,k);
	}
...
void solve(bool SPE){ 
	...
	foru(i,1,n){
		rcnt++;
		int k=RIN;
		tr.modify(rt[rcnt-1],rt[rcnt],1,n,i,k);
	}
	...
	while(m--){
		...
		if(op==1){
			int p=RIN,c=RIN;
			tr.modify(rt[ver[v]],rt[++rcnt],1,n,p,c);
			ver[++vcnt]=rcnt;
		}else{
			int p=RIN;
			printf("%d\n",tr.query(rt[ver[v]],1,n,p));
			
			tr.tr[rt[++rcnt]=++tr.pcnt]=tr.tr[rt[ver[v]]];
			ver[++vcnt]=rcnt;
		}
	}
